...
import IncomeDocumentCheck from "@/components/IncomeDocumentCheck";
    ...
    <main className="mx-auto max-w-xl px-6 py-16">
      ...
      {step === "basics" && (
        <Section title="A few basics" subtitle="This is used only to check eligibility rules.">
          ...
          <Field label="Annual household income (INR)">
            ...
            />
            <IncomeDocumentCheck
              typedIncome={profile.annual_household_income}
              onUseDetected={(v) => update("annual_household_income", v)}
            />
          </Field>
        </Section>
      )}
      ...
      {step === "documents" && (
        <Section title="A couple more details" subtitle="Only asked where a scheme needs it.">
          ...
          <Field label="Date you started street vending (street vendors only — needed for PM SVANidhi)">
            <input
              type="date"
              className="input"
              value={(profile.vending_start_date as string) ?? ""}
              onChange={(e) => update("vending_start_date", e.target.value || undefined)}
            />
          </Field>
          <Field label="Have you taken a similar government loan (PMEGP / MUDRA / SVANidhi) in the last 5 years that's still unpaid? (artisans only — PM Vishwakarma)">
            <YesNoUnset
              value={profile.has_active_similar_government_loan_last_5_years as boolean | undefined}
              onChange={(v) => update("has_active_similar_government_loan_last_5_years", v)}
            />
          </Field>
          <Field label="Is another family member already registered under PM Vishwakarma? (artisans only)">
            <YesNoUnset
              value={profile.has_family_member_registered_in_vishwakarma as boolean | undefined}
              onChange={(v) => update("has_family_member_registered_in_vishwakarma", v)}
            />
          </Field>
        </Section>
      )}
