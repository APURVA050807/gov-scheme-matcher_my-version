...
def load_schemes(path: str = "schemes.json") -> List[Dict[str, Any]]:
    """Loads schemes from a JSON file. Fallback to scheme.json / schemes.json if requested path does not exist."""
    target_path = path
    if not os.path.exists(target_path):
        dir_name = os.path.dirname(target_path)
        base_name = os.path.basename(target_path)
        alt_name = "scheme.json" if base_name == "schemes.json" else "schemes.json"
        alt_path = os.path.join(dir_name, alt_name) if dir_name else alt_name
